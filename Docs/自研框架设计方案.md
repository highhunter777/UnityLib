...
**移动适配的刘海屏红线（2026-09-13 补）**：SafeArea 策略必须覆盖**刘海屏 / 挖孔屏 / 异形屏**——运行期唯一事实源是 `Screen.safeArea`（横竖屏切换、分屏自动更新），UI 画布经 SafeArea 接收器驱动 CanvasScaler 偏移（M4 UI 壳交付）；**IMGUI / 自绘调试层同规则**（DevHUD 已先行落地）；Android 侧出包时决策 `renderInCutout`（默认 shortEdges，挖孔区不放交互），iOS 交由 safeArea 天然覆盖——出包检查单加"异形屏真机过一遍四角"一项。
...
| 技能 | 技能表（**时间轴**/数值/消耗） | **技能系统**：释放/前摇/判定。**单机/表现形态跑在 IGameClock 时序执行器上（头号消费者）；参与联机判定的部分最终住 Sim**（固定步长 + CommandBuffer，见《状态同步实施方案》§3.9 映射） | 可选行为脚本 | 技能栏/CD |
...
| 特效 | 特效表（名→prefab/参数） | **VFX 服务**：加载 + 池化 + 播放接口（模式三） | 触发由技能/对话时序调 | — |
